...
async def get_book(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Вы взяли книгу '{book_name}'. Пожалуйста, верните её позже!"
    )

    # Запускаем таймер напоминания (здесь 60 секунд — можно увеличить)
    asyncio.create_task(remind_return(context.bot, user_id, book_name))
    ...
    if borrowed_books.get(user_id) == book_name:
        try:
            await bot.send_message(
                chat_id=user_id,
                text=(
                    f"Пожалуйста, не забудьте вернуть книгу '{book_name}'. "
                    "Отправьте PDF файла с командой /return."
                )
            )
        except Exception:
