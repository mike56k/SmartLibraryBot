...
class PunishmentSystem:
    ...
    def add_borrow(self, user_id: int, book_name: str):
        ...
        self.borrowed_books[user_id_str] = {
            "book": book_name,
            "borrowed_at": now,
            "fine": 0
        }
    ...
    async def _reminder_loop(self, user_id_str: str):
        ...
        while self._running and user_id_str in self.borrowed_books:
            ...
            if overdue_days > 0:
                msg += f"Срок возврата истек {overdue_days} дн. назад. Штраф: {record['fine']} у.е. Пожалуйста, верните книгу как можно скорее."
            else:
